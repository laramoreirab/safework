...
// GET /pedidos/admin/todas — Retorna todas as vendas para o painel ADM
static async obterTodasVendas(req, res) {
    try {
        const connection = await getConnection();
        ...
        const sql = `
            SELECT 
                ...
                dp.nome_representante,
                dp.metodo_pagamento,
                (SELECT COUNT(*) FROM itens_pedidos ip WHERE ip.pedido_id = p.id) AS total_itens,
                (SELECT p2.nome 
                 FROM itens_pedidos ip2 
                 JOIN produtos p2 ON ip2.produto_id = p2.id 
                 WHERE ip2.pedido_id = p.id 
                 ORDER BY ip2.id ASC LIMIT 1) AS primeiro_item_nome
            FROM pedidos p
            LEFT JOIN dados_pedido dp ON p.id = dp.pedidoId
            LEFT JOIN empresas e ON p.usuario_id = e.id
            WHERE p.status IN ('enviado', 'pago')
            ORDER BY p.data_criacao DESC
        `;
        ...
        return res.status(200).json({ sucesso: true, dados: pedidos });
