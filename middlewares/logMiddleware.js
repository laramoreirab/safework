...
export const logMiddleware = async (req, res, next) => {
    ...
    const logData = {
        ...
        dados_requisicao: JSON.stringify({
            ...
            cookies: req.cookies ? Object.keys(req.cookies).reduce((acc, key) => {
            acc[key] = key.toLowerCase().includes('token') ? '[REDACTED]' : req.cookies[key];
            return acc;}, {}) : null, // Se o cookie tiver "token" no nome, mascara o valor do token
            //Isso garante que você não loga tokens sensíveis, mas ainda registra outros cookies úteis
